...
class ClientesController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    override func setEditing(_ editing: Bool, animated: Bool) {
        ...
        if  editing{
            ...
            self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Delete", style: .plain, target: self, action: #selector(deleteSelectdItemAction(_ :)))
        }
        else{
        ...
        if  let IndexPaths = collectionView?.indexPathsForSelectedItems {
            for IndexPath in IndexPaths {
                if let cell = collectionView?.cellForItem(at: IndexPath) as? ClienteCell{
                    ...
                    reference(to: .clientes).whereField("email", isEqualTo: cell.cliente?.email).getDocuments() { (querySnapshot, error) in
                        print(querySnapshot)
                        print(error)
                    }
